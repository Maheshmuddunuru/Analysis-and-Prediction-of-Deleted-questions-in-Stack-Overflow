...
df =  pd.read_csv(r"E:\project papers cite\NOTES\combineddeleteddf.csv")
df2= pd.DataFrame()
######################################## TAGS FEARURES####################
feature_no_of_tags = []
########################################
...
for i in df.Body:
    ...
    #feature_text_standard_body.append(textstat.text_standard(i))
...
for i in df.Title:
    ...
    #feature_text_standard_body_title.append(textstat.text_standard(i))
...
for tags in df.Tags:
    lent=0
    lent = sum(1 for word in tags.split() if word.isalpha())
    feature_no_of_tags.append(lent)
...
#df2['text_standard_body']=feature_text_standard_body
...
#df2['text_standard_body_title']=feature_text_standard_body_title
...
df2.to_csv('testfeaturedistribution.csv',index=False)
